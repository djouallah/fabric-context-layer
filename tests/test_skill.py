...
- `client/SKILL.md` - **everywhere else**. The same file is installed under
...
# The M365 agent's lookup: DAX over the context model's own tables (client/m365/instructions.md).
...
def test_both_skills_carry_the_frontmatter_the_agents_discover_by():
    for path in (SKILL, CLIENT_SKILL):
