...
three names are the same in every tenant on purpose: `client/` has no clone and no config, so
...
keeps `client/SKILL.md` spelling both names the way the harvest writes them.
